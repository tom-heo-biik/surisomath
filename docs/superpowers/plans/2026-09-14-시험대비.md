...
```
Expected: `[[{'x': 60.0, 'y': 116.0, 'w': 475.0, 'h': 0.0, 'class': 'work box'}]]` — 본문 위 72pt에 첫 문단의 글줄 22pt와 문단 아래 여백 22pt를 더한 116pt(px→pt 환산이라 474.99…로 찍혀도 된다). img가 없으니 상자는 이것 하나다. (`$TMP`는 스크래치 폴더.)
