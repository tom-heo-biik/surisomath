...
def p2():
    ...
    f, ax = g.canvas(8, -8.1, 8.5)
    arrow(ax, (-R - 0.6, 0), (R + 0.8, 0))
    arrow(ax, (0, -R - 0.6), (0, R + 0.8))
    g.label(ax, R + 1.1, 0, "$x$", ha="left")
    g.label(ax, -0.45, R + 0.45, "$y$", ha="right")     # 축 끝 왼쪽. 위에 두면 글자 한 줄만큼 그림이 작아진다
...
def p3():
    a, P = -12.0, (-4.0, 3.0)
    R = 7.0
    f, ax = g.canvas(6, -8.27, 8.68)
    arrow(ax, (-R - 0.6, 0), (R + 0.8, 0))
    arrow(ax, (0, -R - 0.6), (0, R + 0.8))
    g.label(ax, R + 1.1, 0, "$x$", ha="left")
    g.label(ax, 0.45, R + 0.45, "$y$", ha="left")      # 축 끝 오른쪽 — 왼쪽 위에는 가지 끝이 있다
