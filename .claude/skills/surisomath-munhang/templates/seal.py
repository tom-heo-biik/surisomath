...
def check(yaml_path: Path, data: dict) -> list[str]:
    """봉인과 지금 글을 대조해 어긋난 문항마다 경고 글을 돌려준다. 봉인이 없으면 빈 목록."""
...
def main(argv: list[str]) -> int:
    ...
    for no in nos:
        if no not in now:
            ...
            return 1
        seals[no] = {"date": today, "digest": digest(now[no])}
        print(f"  {no}: 봉인 {today} {seals[no]['digest']}")
    save(yaml_path, seals)
    return 0
